...
class OrientationVisualizer:
    def __init__(self, stl_file):
        ...
        self.window.setWindowTitle("3D Rocket Orientation Visualizer")
        ...
        def on_json_response(data):
            try:
                ...
                self.latest_quaternion = (
                    float(quaternion.get('4', 0)),  # w
                    float(quaternion.get('1', 0)),  # x
                    float(quaternion.get('2', 0)),  # y
                    float(quaternion.get('3', 0))   # z
                )
            except (ValueError, KeyError) as e:
                print(f"Error processing quaternion data: {e}")

    def update_display(self):
        """Update the orientation of the object based on latest quaternion data."""
        if self.latest_quaternion:
            try:
                qw, qx, qy, qz = self.latest_quaternion
                rotation_matrix = self.quaternion_to_matrix(qw, qx, qy, qz)
                self.object.setTransform(QtGui.QMatrix4x4(*rotation_matrix.flatten()))
            except Exception as e:
                print(f"Error updating orientation: {e}")
